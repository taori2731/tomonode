...
(async () => {
  ...
  const page = await browser.newPage({ viewport: { width: 1580, height: 980 }, colorScheme: "dark" });
  ...
  if (await page.locator(".app").getAttribute("data-theme") !== "dark") throw new Error("未設定テーマがダークで始まりませんでした");
  const titlebarLogo = page.locator(".titlebar .brand-mark");
  if (await titlebarLogo.getAttribute("src") !== "/assets/tomonode-icon-bg-black.png") throw new Error("ダークテーマに黒背景ロゴが表示されませんでした");
  if (await titlebarLogo.getAttribute("alt") !== "" || await titlebarLogo.getAttribute("aria-hidden") !== "true") throw new Error("タイトルバーの装飾ロゴがアクセシビリティツリーから除外されていません");
  ...
  if (await titlebarLogo.getAttribute("src") !== "/assets/tomonode-icon-bg-white.png") throw new Error("ライトテーマに白背景ロゴが表示されませんでした");
  ...
  if (await titlebarLogo.getAttribute("src") !== "/assets/tomonode-icon-bg-black.png") throw new Error("システムのダーク設定に黒背景ロゴが追従しませんでした");
  ...
  for (let attempt = 0; attempt < 3 && await page.locator(".app").getAttribute("data-theme") !== "light"; attempt += 1) await page.locator(".theme-button").evaluate((button) => button.click());
